...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    out = np.expand_dims(gamma, axis=0) * x_normalized + np.expand_dims(beta, axis=0)
    cache = None
...
def batchnorm_backward(dout, cache):
  ...
  dvar = np.sum(dx_normalized * (x - sample_mean) * -0.5 * np.power(sample_var + eps, -1.5), axis=0)
  dmean = np.sum(dx_normalized * -1 / np.sqrt(sample_var + eps), axis=0) + dvar * np.mean(-2 * (x - sample_mean), axis=0)
